...
public class AbilityUIManager : MonoBehaviour
{
    ...
    private IEnumerator ShowAbilityCoroutine(Sprite portrait, string name, float duration)
    {
        ...
        if (abilityPortrait == null)
            abilityPortrait = GameObject.Find("HabilitiePortrait")?.GetComponent<Image>();
        if (abilityName == null)
            abilityName = GameObject.Find("HabilitieName")?.GetComponent<TMPro.TMP_Text>();

        // Atualizar UI
        if (abilityPortrait != null)
            abilityPortrait.sprite = portrait;
        if (abilityName != null)
            abilityName.text = "Nova Habilidade Desbloqueada: " + name;
